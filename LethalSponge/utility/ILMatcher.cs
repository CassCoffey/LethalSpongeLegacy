...
/**
 * Thank you Zaggy1024 for suggesting I use your ILInjector over CodeMatcher.
 * All of the code in this file is from https://github.com/Zaggy1024/LC_PathfindingLagFix
 * Licensed under MIT License by Zaggy1024
 */
public interface ILMatcher
{
    ...
    {
        if (method == null)
            Plugin.Log.LogWarning($"Method passed to ILMatcher.Callvirt() was null\n{new StackTrace()}");
        return OpcodeOperand(OpCodes.Callvirt, method);
    }
    public static OpcodeOperandMatcher Call(MethodBase method)
    {
        if (method == null)
            Plugin.Log.LogWarning($"Method passed to ILMatcher.Call() was null\n{new StackTrace()}");
